...
class 按钮 {
    ...
    public function 弹窗(string $内容, string $取消文本 = '我知道了', ?string $确认文本 = null): self {
        $this->数据['action']['modal'] = [
            'content' => $内容,
            'cancel_text' => $取消文本
        ];
        if ($确认文本 !== null) {
            $this->数据['action']['modal']['confirm_text'] = $确认文本;
        }
        $this->数据['action']['type'] = 1;
        return $this;
    }
    ...
    public static function 小字体(array ...$行数组): string {
        $行们 = [];
        foreach ($行数组 as $一行按钮) {
            if (isset($一行按钮[0]) && !isset($一行按钮[0]['id'])) {
                $行们[] = ['buttons' => $一行按钮];
